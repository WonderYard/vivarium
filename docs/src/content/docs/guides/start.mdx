...
<Aside type="note" title="A gentler introduction">
  If you're not familiar with cellular automata, we suggest reading the [Foundations](/learn/intro)
  page before getting started writing code with Vivarium.
</Aside>
...
By default, the grid is **non-wrapping** — cells at the borders have fewer neighbors. If you want a toroidal grid where edges connect to the opposite side, pass `{ wrapping: true }` as the second argument:

```typescript
const vi = vivarium("square", { wrapping: true });
```

<Aside type="note" title="Power-of-2 constraint">
  When wrapping is enabled, the canvas width and height must be powers of 2 (e.g. 64, 128, 256).
</Aside>
...
<Aside type="tip" title="Image rendering">
  Setting `imageRendering` to `"pixelated"` ensures cells appear crisp when the canvas is scaled up
  in CSS, rather than blurry.
</Aside>
