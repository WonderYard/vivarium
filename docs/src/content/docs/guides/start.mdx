...
<Aside type="note" title="A gentler introduction">
  If you're not familiar with cellular automata, we suggest you to read the
  [Foundations](/learn/intro) page before getting started writing code with
  Vivarium.
</Aside>
...
<Tabs syncKey="package-manager">
  <TabItem label="npm" icon="seti:npm">
    ```bash frame=none npm i @wonderyard/vivarium ```
  </TabItem>
  <TabItem label="pnpm" icon="pnpm">
    ```bash frame=none pnpm add @wonderyard/vivarium ```
  </TabItem>
  <TabItem label="yarn" icon="seti:yarn">
    ```bash frame=none yarn add @wonderyard/vivarium ```
  </TabItem>
</Tabs>
...
<Aside type="tip" title="Image rendering">
  Setting `imageRendering` to `"pixelated"` ensures cells appear crisp when the
  canvas is scaled up in CSS, rather than blurry.
</Aside>
...
Optionally, you can pass an `initialGrid` to the `setup` function. `initialGrid` is a list of element indices that will pre-populate the canvas. The list can be 1d (flat list of indices) or 2d (list of rows of indices).
