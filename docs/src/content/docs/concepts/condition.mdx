...
space.to(cat).count(cat, between(1, 5));
// same as
space.to(cat).count(cat, [1, 2, 3, 4, 5]);
```
...
space.to(cat).count(cat, not(2));
// in a square neighborhood the line above is equivalent to:
space.to(cat).count(cat, [0, 1, 3, 4, 5, 6, 7, 8]);
```
...
space.to(cat).count(cat, even());
// same as
space.to(cat).count(cat, [0, 2, 4, 6, 8]);

space.to(cat).count(cat, odd());
// same as
space.to(cat).count(cat, [1, 3, 5, 7]);
```
...
space.to(cat).is(neighbor.TOP, cat);
```

```typescript
const { neighbor } = vi;

space.to(cat).is(neighbor.TOP, animal);
```

```typescript
const { neighbor } = vi;

space.to(cat).is(neighbor.TOP, neighbor.BOTTOM);
```

### Chance

```typescript
// 1 in 1000 chance for the cell to become a cat
space.to(cat).chance(1, 1000);
```
...
```typescript
space.to(cat).count(cat, 1).chance(1, 1000).accept("any")
```
