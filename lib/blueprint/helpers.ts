...
export class Helpers {
   ...
   * @param max - The end of the range (must be ≤ 8 and greater than `min`). For cross neighborhoods, should not exceed 4. For hexagonal, should not exceed 6.
   ...
   *
   * @returns `[0, 2, 4, 6, 8]` for square, `[0, 2, 4]` for cross, `[0, 2, 4, 6]` for hexagonal.
   */
  public even(): number[] {
    return this.neighborhoodName === "cross"
      ? [0, 2, 4]
      : this.neighborhoodName === "hexagonal"
        ? [0, 2, 4, 6]
        : [0, 2, 4, 6, 8];
   ...
   *
   * @returns `[1, 3, 5, 7]` for square, `[1, 3]` for cross, `[1, 3, 5]` for hexagonal.
   */
  public odd(): number[] {
    return this.neighborhoodName === "cross"
      ? [1, 3]
